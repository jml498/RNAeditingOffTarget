...
function scoreAlignment(ppr, target, offset, scoring_tables)
    ...
    for (i, (motif_type, motif)) in enumerate(zip(ppr.motif_types, ppr.motifs))
        ...
        push!(nucleotide_scores, (i, nt, score))  # Include position and nucleotide in the score
        sum_score += score
    end
    return sum_score, nucleotide_scores
end
...
function scoreAllAlignments(ppr, target, scoring_tables)
    ...
    nucleotide_scores_list = Vector{Vector{Tuple{Int, Char, Float64}}}(undef, maxoffset)
    for offset in 1:maxoffset
        score, nucleotide_scores = scoreAlignment(ppr, target, offset, scoring_tables)
        scores[offset] = score
        nucleotide_scores_list[offset] = nucleotide_scores
    end
    return scores, nucleotide_scores_list
end
...
function main()
    ...
    if !isnothing(ref_file)
        ...
        open(ref_file) do reff
            ...
            for (i, ppr) in enumerate(pprs)
                fscores, _ = scoreAllAlignments(ppr, reference, scoring_tables)
                rscores, _ = scoreAllAlignments(ppr, revcomp(reference), scoring_tables)
    ...
    # Determine maximum length of motifs to set up column headers
    headers = ["protein_ID", "target_ID", "start", "end", "aligned_nucleotides", "score"]
    nucleotide_score_labels = ["nucleotide_score_1", "nucleotide_score_2", "nucleotide_score_3", "nucleotide_score_4", "nucleotide_score_6", "nucleotide_score_7", "nucleotide_score_8", "nucleotide_score_9", "nucleotide_score_10", "nucleotide_score_12"]
    append!(headers, nucleotide_score_labels)
    if using_reference
        push!(headers, "Zscore")
    end
    ...
    for (i, ppr) in enumerate(pprs), target in targets
        if edit_site > 0
            ...
            if offset < 1 || offset + length(ppr.motifs) - 1 > length(target.seq)
                continue
            end
            score, nucleotide_scores = scoreAlignment(ppr, target, offset, scoring_tables)
            row = [ppr.id, target.id, offset, offset + length(ppr.motifs) - 1, target.seq[offset:offset + length(ppr.motifs) - 1], score]
            for (pos, nt, score) in nucleotide_scores
                push!(row, "$pos:$nt:$score")
            end
            # Fill in missing columns if nucleotide_scores is less than 10
            while length(row) < length(headers)
                push!(row, "")
            end
            if using_reference
                push!(row, (score - ref_means[i]) / ref_stds[i])
            end
            write(io, join(row, "\t"))
            write(io, "\n")
        else
            scores, nucleotide_scores_list = scoreAllAlignments(ppr, target, scoring_tables)
            for (pos, score) in enumerate(scores)
                offset = pos + 1
                nucleotide_scores = nucleotide_scores_list[pos]
                row = [ppr.id, target.id, offset, offset + length(ppr.motifs) - 1, target.seq[offset:offset + length(ppr.motifs) - 1], score]
                for (pos, nt, score) in nucleotide_scores
                    push!(row, "$pos:$nt:$score")
                end
